...
def test_waypoint_location():
    ...
    assert result.x, result.y == pytest.approx((lon1, lat1))

    result = GeoHelper.waypoint_location(lat1, lon1, lat2, lon2, distance_m = 162.5e3)
    assert result.x, result.y == pytest.approx((lon2, lat2), 0.01)

    result = GeoHelper.waypoint_location(lat1, lon1, lat2, lon2, distance_m = 81.25e3)
    assert result.x, result.y == pytest.approx((-1.051272, 51.06276), 0.01)
    ...
    assert result.x, result.y == pytest.approx((-2.9125, 50.1154), 0.01)
